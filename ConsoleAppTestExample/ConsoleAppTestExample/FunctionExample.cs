...
namespace ConsoleAppTestExample
{
    public class FunctionExample
    {
        ...
        // Other Class
        #region 
        public static void Swap(Product[] array, int i, int j)
        {
            Product temp = null;
        ...
        public static void Test(List<Product> listProduct, string nameProduct, int categoryId, int price, Product[] listProductByArray, int example)
        {
            if (example == 4)
            {
                Console.WriteLine("Product(name: " + FunctionExample.FindProduct(listProduct, nameProduct).Name + ", price: " +
                "" + FunctionExample.FindProduct(listProduct, nameProduct).Price + ", quality: " +
                "" + FunctionExample.FindProduct(listProduct, nameProduct).Quality + ", categoryId: " +
                "" + FunctionExample.FindProduct(listProduct, nameProduct).CategoryId + ")");
            }
            else if (example == 5)
            {
                foreach (var item in FunctionExample.FindProductByCategory(listProduct, categoryId))
                {
            ...
            {
                foreach (var item in FunctionExample.FindProductByPrice(listProduct, price))
                {
                    Console.WriteLine("Product(name: " + item.Name + ", price: " +
                                    "" + item.Price + ", quality: " +
                                    "" + item.Quality + ", categoryId: " +
